      ...
      <main className="max-w-md mx-auto px-4 py-8 relative">
        ...
        {appState === AppState.LOADING && (
          <div className="flex flex-col items-center justify-center pt-20 space-y-4">
            ...
            <h3 className="text-lg font-black text-gray-600 tracking-tight">ANALISANDO MERCADO...</h3>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest animate-pulse">Consultando FIPE e Anúncios Reais</p>
          </div>
        )}
        {appState === AppState.RESULT && result && lastFormData && <AnalysisResult data={result} vehicleData={lastFormData} onReset={resetApp} />}
        {appState === AppState.ERROR && (
          <div className="p-6 text-center space-y-4 bg-white rounded-2xl border border-red-100 shadow-sm">
            <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mx-auto text-red-600">
              <Star className="w-6 h-6" />
            </div>
            <h3 className="text-slate-900 font-black uppercase text-sm tracking-tighter">Falha na Inteligência</h3>
            <p className="text-xs text-gray-500 leading-relaxed">{error}</p>
            <button onClick={resetApp} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold shadow-lg shadow-slate-200">Tentar Novamente</button>
          </div>
        )}
      </main>
    </div>
