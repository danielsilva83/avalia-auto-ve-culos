...
import { AnalysisResponse, VehicleFormData } from '../types';
import { 
...
import { generateToolContent, ToolType as ServiceToolType } from '../services/geminiService';

type ToolType = 'dossier' | 'ads' | 'future' | 'negotiation' | 'profit' | null;
...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ data, vehicleData, onReset }) => {
  ...
  const openTool = async (type: ToolType) => {
    setActiveTool(type);
    setShowMenu(false);
    if (type === 'profit' || !type) return; // Calculadora é local ou tipo inválido

    setIsToolLoading(true);
    try {
      const content = await generateToolContent(type as ServiceToolType, vehicleData);
  ...
  // --- FERRAMENTA: CALCULADORA DE LUCRO (LOCAL) ---
    ...
    return (
      <div className="space-y-6 animate-fade-in-up">
        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase block mb-1">Gastos (Mecânica/Estética)</label>
            <div className="relative">
          ...
          <div className="pt-2 border-t flex justify-between items-center">
            <span className="text-xs font-bold text-slate-500 uppercase">Preço de Venda (IA):</span>
        ...
        <div className="p-4 bg-slate-900 text-white rounded-xl text-[11px] leading-relaxed italic opacity-80">
          * Cálculo baseado no valor de mercado sugerido pela IA para o estado de {vehicleData.uf}.
        </div>
      </div>
    );
  };

  // --- OVERLAY DE CONTEÚDO DAS FERRAMENTAS ---
  const ToolOverlay = () => {
    ...
    const titles: Record<string, string> = {
      ...
      negotiation: 'Battle Cards de Negociação',
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-6 pb-32">
      ...
      {/* Sumário Regional de Preços */}
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-900 p-5 rounded-2xl text-white shadow-xl shadow-slate-900/10">
           <p className="text-[10px] font-black opacity-50 uppercase tracking-widest mb-1">Sugestão de Preço</p>
      ...
      {/* Inteligência de Mercado (Seção 1) */}
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="bg-slate-50 px-6 py-4 flex items-center justify-between border-b border-slate-100">
           <div className="flex items-center gap-2">
             <Target className="w-4 h-4 text-blue-600" />
             <h3 className="font-black text-slate-900 uppercase text-[10px] tracking-widest">Análise Regional</h3>
           </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-50">
              ...
              <div className="flex flex-wrap gap-2">
                {data.groundingUrls.map((link, i) => (
                  <a key={i} href={link.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-lg text-[11px] text-slate-600 font-bold hover:bg-slate-100 hover:text-blue-600 transition-all border border-slate-100 max-w-full">
      ...
      {/* Menu flutuante de Ferramentas Estratégicas */}
      {showMenu && (
        <div className="fixed inset-0 z-50 flex items-end p-4 animate-fade-in">
           ...
           <div className="relative w-full bg-white rounded-3xl overflow-hidden animate-fade-in-up shadow-2xl">
              <div className="p-6 border-b flex justify-between items-center bg-slate-50">
                ...
                <button onClick={() => setShowMenu(false)} className="p-2 bg-white border border-slate-200 rounded-full hover:rotate-90 transition-transform shadow-sm">
              ...
              <div className="p-3 grid gap-2 max-h-[70vh] overflow-y-auto">
                <button onClick={() => openTool('dossier')} className="w-full p-4 flex items-center gap-4 bg-slate-50 rounded-2xl hover:bg-blue-50 transition-all text-left group border border-transparent hover:border-blue-100">
                  <div className="p-3 bg-blue-100 text-blue-600 rounded-xl group-hover:scale-110 transition-transform shadow-inner"><FileText className="w-6 h-6" /></div>
                  <div className="flex-1">
                    <p className="font-bold text-slate-900">Dossiê de Venda</p>
                    <p className="text-[10px] text-slate-400 uppercase font-black">Laudo Visual para Cliente</p>
                  </div>
                ...
                <button onClick={() => openTool('ads')} className="w-full p-4 flex items-center gap-4 bg-slate-50 rounded-2xl hover:bg-orange-50 transition-all text-left group border border-transparent hover:border-orange-100">
                  <div className="p-3 bg-orange-100 text-orange-600 rounded-xl group-hover:scale-110 transition-transform shadow-inner"><Megaphone className="w-6 h-6" /></div>
                  <div className="flex-1">
                ...
                <button onClick={() => openTool('future')} className="w-full p-4 flex items-center gap-4 bg-slate-50 rounded-2xl hover:bg-purple-50 transition-all text-left group border border-transparent hover:border-purple-100">
                  <div className="p-3 bg-purple-100 text-purple-600 rounded-xl group-hover:scale-110 transition-transform shadow-inner"><TrendingDown className="w-6 h-6" /></div>
                  <div className="flex-1">
                    <p className="font-bold text-slate-900">Visão de Futuro</p>
                    <p className="text-[10px] text-slate-400 uppercase font-black">Projeção de 24 Meses</p>
                  </div>
                  <ChevronRight className="w-5 h-5 text-slate-300" />
                </button>
                <button onClick={() => openTool('negotiation')} className="w-full p-4 flex items-center gap-4 bg-slate-50 rounded-2xl hover:bg-red-50 transition-all text-left group border border-transparent hover:border-red-100">
                  <div className="p-3 bg-red-100 text-red-600 rounded-xl group-hover:scale-110 transition-transform shadow-inner"><ShieldAlert className="w-6 h-6" /></div>
                  <div className="flex-1">
                    <p className="font-bold text-slate-900">Cards de Negociação</p>
                    <p className="text-[10px] text-slate-400 uppercase font-black">Objeções e Respostas</p>
                  </div>
                  <ChevronRight className="w-5 h-5 text-slate-300" />
                </button>
                <button onClick={() => openTool('profit')} className="w-full p-4 flex items-center gap-4 bg-slate-50 rounded-2xl hover:bg-green-50 transition-all text-left group border border-transparent hover:border-green-100">
                  <div className="p-3 bg-green-100 text-green-600 rounded-xl group-hover:scale-110 transition-transform shadow-inner"><Calculator className="w-6 h-6" /></div>
                  <div className="flex-1">
                    <p className="font-bold text-slate-900">Calculadora de Lucro</p>
                    <p className="text-[10px] text-slate-400 uppercase font-black">Margem Líquida de Revenda</p>
                  </div>
                  <ChevronRight className="w-5 h-5 text-slate-300" />
                </button>
              </div>
              <div className="p-6 bg-slate-50 text-center">
                 <p className="text-[10px] font-black text-slate-400 uppercase">Aumente sua lucratividade com dados reais.</p>
              </div>
           </div>
        </div>
      )}

      {/* Ações Flutuantes Fixas */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur-md border-t border-slate-100 z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <div className="max-w-md mx-auto flex gap-3">
          <a href={`https://wa.me/?text=${generateWhatsAppText()}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-green-600 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-green-900/20 active:scale-95 transition-transform hover:bg-green-700">
            <Share2 className="w-5 h-5" /> COMPARTILHAR
          </a>
          <button onClick={() => setShowMenu(true)} className="flex-1 bg-slate-900 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-slate-900/20 active:scale-95 transition-transform hover:bg-slate-800">
